...
declare global {
      var signUpAndGetCookie: (id?: string) => string[];  // A cookie is a string[]
}
...
// we define this as soon as possible. we cant wait until the beforeAll call. it's going to be needed and used as soon as we require in / import the stripe library in our test script.
process.env.STRIPE_KEY = 'sk_test_51NrkU9Ir5VklcuQgxGZQxwHEKEOhuaAVlg4MeWEQ6K5ezgJTGSQxf1sRMYGQFhABGFc1ssOSdYt7Chfx8A2blauo00GGhty10h';
...
global.signUpAndGetCookie = (id?: string) => {
  ...
  const payload = {
    id: id || new mongoose.Types.ObjectId().toHexString(),   // Use the provided id if provided, otherwise generate one
